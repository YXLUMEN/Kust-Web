function checkPassword() {
    const rePasswordArea = document.getElementById("rePasswordReg");

    const passwordValue = document.getElementById("passwordReg").value;
    const rePasswordValue = document.getElementById("rePasswordReg").value;
...
function checkStrong() {
    ...
    const pattern = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[!@#$%^&*()_+\-={}|:"<>?,./;'`~\[\]\\])[A-Za-z\d!@#$%^&*()_+\-={}|:"<>?,./;`~'\[\]\\]{6,20}$/;
